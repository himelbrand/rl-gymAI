...
MAX_STEPS = 1000
png_suffix = ''
P_CENTERS = [-0.9,-0.6,-0.3,0.3]
V_CENTERS = [-0.049, -0.035, -0.021, -0.007, 0.007, 0.021, 0.035, 0.049] #[(i+0.5)*0.014 for i in range(-4,4)]
...
def save_run(pi,x,y,alpha,Lambda):
    try:
        with open(f'out/{alpha}-{Lambda}-x.npy','wb') as f:
            np.save(f,np.array(x))
        with open(f'out/{alpha}-{Lambda}-y.npy','wb') as f:
            np.save(f,np.array(y))
        with open(f'out/{alpha}-{Lambda}-pi.npy','wb') as f:
            np.save(f,pi)
    except:
    ...
    for j,(s,l,a) in enumerate(product(sizes,lambdas,alphas)):
        ...
        for label in values:
            ...
            if alpha in a and Lambda in l:
                ...
                plt.legend()
                plt.savefig(f'out/plots/plot({datetime.strftime(datetime.now(),"%d-%m_%H:%M")})_{min(s)}-{max(s)}{png_suffix}_{j}.png')
        plt.close()
def human_agent(env):
